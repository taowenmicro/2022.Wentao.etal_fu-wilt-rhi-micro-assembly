...
phypath = paste(res1path,"/result1_2/",sep = "")
dir.create(phypath)
...
result = neutralModel(ps = ps16s,group  = "Group",ncol = 5,nrow  = 2)
#--合并图表
p1 =  result[[1]]
p1
# #--分开的图表--存储在一个list中
...
ggsave(FileName, p1,width = 16,height = 8)
...
result = bNTICul(ps = ps16s1,group  = "Group",num = 10,thread = 1)
...
result = bNTIRCPlot(ps = ps16s1 ,RCb  = RCb,bNTI = bNTI,group  = "Group")
...
ggsave(FileName, p5,width = 18,height = 8)
